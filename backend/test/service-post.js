...
function addPosts (u, pArr) {
    pArr.forEach(async (element) => {
        ...
        await Service.addPost(u, element)
    })
...
describe.only('postService', () => {
    ...
    describe('.addPost', () => {
        it('normal, add', async () => {
            ...
            expect(arrify(a.post)).to.have.deep.members(arrify(addPostMiscelFields(p1)))
    ...
    describe('.getPosts', () => {
        it('get all of mine and friends except friends\' private post', async () => {
            addPosts(u1, [fr1, pb1, pr1])
            addPosts(u2, [fr2, pb2, pr2])
            addPosts(u3, [fr3, pb3, pr3])

            let a = await Service.getPosts(u1, [u2, u3])

            let posts = a.map((el) => { return el.post })
            posts = posts.map((e) => { return { description: e.description, media: e.media, level: e.level } })
    ...
    describe('getPublicPosts', () => {
        it('get lastests \'n\' public posts', async () => {
            await addPosts(u1, [pr1, pb1, pb2])
            await addPosts(u2, [pr2, pb1, pb2])
            await addPosts(u3, [pr3, pb1, pb2])

            let a = await Service.getPublicPosts(2)
            a = a.map(e => { return e.post })
            a = a.map((e) => { return { description: e.description, media: e.media, level: e.level } })
            // expect.
